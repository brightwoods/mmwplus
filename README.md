...
✔ **연결끊김 방지**  : 인터넷 오류 시 정상화될 때까지 대기 & 새로고침

✔ **재생유지**  : □□ 우회 기능 (봉준 카페 **건빵** 이상 인증 후 기능 활성화)

✔ **모바일 환경 지원** : 실사용 하지 않는 안드로이드 폰을 스밍 전용기기로 활용 가능
...
**✔ 버그픽스 (v2.8.2)** : 인증코드 발급 서버의 보안연결 문제로 해외 거주자나 특수IP 사용자가 스크립트를 사용할 수 없는 문제를 수정 했습니다.

**✔ 새 기능(v2.8.0)** : 모바일 환경 지원 👇🏻 [모바일 환경 설치 바로가기](#2-모바일-환경-설치)
